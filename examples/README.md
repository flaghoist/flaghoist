...
| [`docker`](./docker) | Run the server as a portable container on any host, with env-configured storage (Postgres, Redis, or memory).                         | `docker build -t flaghoist .` (in `examples/docker`) |
